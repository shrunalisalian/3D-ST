...
from models import FeatureExtractor, PointCloudDecoder
...
def model_pass(teacher, decoder, point_cloud):
    ...
    """
    if point_cloud.dim() == 2: # Check if the tensor has two dimensions (N, D)
        point_cloud = point_cloud.unsqueeze(0) # Add a batch dimension (1, N, D)

    point_cloud = point_cloud.to(DEVICE)  # Move point cloud to the correct device

    # Debug print to verify shape of point_cloud
    print(f"point_cloud shape before computing geometric features: {point_cloud.shape}")
...
# Training loop
def train(train_data, val_data, teacher, decoder, optimizer, num_epochs, save_dir, batch_size=32, scaler = None):
    """
    ...
    Args:
        ...
        scaler (GradScaler): GradScaler object for mixed precision training.
    """
    ...
    for epoch in pbar:
        ...
        # for point_clouds in torch.split(train_data, batch_size):
        #     optimizer.zero_grad()
        ...
        #         loss.backward()
        #     optimizer.step()
        #     epoch_training_loss += loss.item() * point_clouds.size(0)
        ...
        with torch.no_grad(): # Disable gradient computation for validation
            for point_clouds in torch.split(val_data, batch_size):
                for point_cloud in point_clouds:
                    # Debugging: Print the shape of point_cloud
                    print(f"Validation epoch {epoch}, shape of point_cloud: {point_cloud.shape}")
                    loss, _ = model_pass(teacher, decoder, point_cloud)
...
def main():
    ...
    train(train_data, val_data, teacher, decoder, optimizer, NUM_PRETRAINING_EPOCHS, save_dir='../training_saves', batch_size=32, scaler=scaler)
