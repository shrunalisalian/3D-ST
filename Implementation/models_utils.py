...
def calculate_pairwise_distances(points: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    """
    B, N, _ = points.shape # points: A tensor of shape (B, N, D) 
    points_1 = points.unsqueeze(2) # points_1 becomes of shape (B, N, 1, D) by adding a new dimension at index 2.
    points_2 = points.unsqueeze(1) # points_2 becomes of shape (B, 1, N, D) by adding a new dimension at index 1.
    diffs = points_1 - points_2 # points_1 and points_2 are broadcasted to shape (B, N, N, D).
    distances = torch.norm(diffs, dim=-1) # distances[b, i, j] is the Euclidean distance between the i-th and j-th points in the b-th point cloud.
...
def find_closest_neighbors(distances: torch.Tensor, num_neighbors: int) -> torch.Tensor:
    ...
    """
    B, N, _ = distances.shape # distances[b, i, j] represents the distance between the i-th and j-th points in the b-th point cloud
    if num_neighbors >= N:
    ...
    return nearest_indices # a tensor of shape (B, N, num_neighbors)
...
#         points_batch = points[i:i + batch_size] # points_batch has shape (min(batch_size, B - i), N, D)

#         # diffs has shape (batch_size, N, N, D)  containing the difference vectors.
#         # distances has shape (batch_size, N, N) containing the Euclidean distances.
#         # Compute the pairwise Euclidean distances and differences between all points within each batch.
#         diffs, distances = calculate_pairwise_distances(points_batch) 

#         # nearest_indices has shape (batch_size, N, num_neighbors), containing the indices of the nearest neighbors.
#         # Identify the closest neighbors for each point within the batch.
#         nearest_indices = find_closest_neighbors(distances, num_neighbors)

#         # batch_indices has shape (batch_size, N, num_neighbors).
#         # batch_indices creates an index tensor to select the appropriate batch elements.
...
#         # vector_diffs has shape (batch_size, N, num_neighbors, D).
#         # vector_diffs extracts the difference vectors for the nearest neighbors.
#         vector_diffs = diffs[batch_indices, nearest_indices]

#         # scalar_diffs has shape (batch_size, N, num_neighbors).
#         # scalar_diffs extracts the scalar distances for the nearest neighbors.
#         scalar_diffs = distances[batch_indices, nearest_indices]

#         # scalar_diffs is expanded to match the dimension of vector_diffs for concatenation.
#         # scalar_diffs now has shape (batch_size, N, num_neighbors, 1).
#         scalar_diffs = scalar_diffs.unsqueeze(-1)

#         # geom_features is created by concatenating vector_diffs and scalar_diffs along the last dimension.
#         # geom_features has shape (batch_size, N, num_neighbors, D + 1).
...
#     # geom_features has shape (B, N, num_neighbors, D + 1).
#     geom_features = torch.cat(geom_features_list, dim=0)
#     # nearest_indices has shape (B, N, num_neighbors).
...
def compute_geometric_features(points: torch.Tensor, num_neighbors: int, batch_size: int = 1) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    for i in range(0, B, batch_size):
        points_batch = points[i:i + batch_size]  # points_batch has shape (min(batch_size, B - i), N, D)
        print(f"points_batch shape: {points_batch.shape}")

        # Compute the pairwise Euclidean distances and differences between all points within each batch.
        diffs, distances = calculate_pairwise_distances(points_batch)
        print(f"diffs shape: {diffs.shape}, distances shape: {distances.shape}")

        # Identify the closest neighbors for each point within the batch.
        nearest_indices = find_closest_neighbors(distances, num_neighbors)
        print(f"nearest_indices shape: {nearest_indices.shape}")

        # Create batch indices for selecting appropriate elements.
        batch_indices = torch.arange(points_batch.shape[0]).view(-1, 1, 1).expand(-1, N, num_neighbors).to(points.device)
        print(f"batch_indices shape: {batch_indices.shape}")

        # Extract the difference vectors and distances for the nearest neighbors.
        vector_diffs = diffs[batch_indices, torch.arange(N).view(1, -1, 1).expand(B, -1, num_neighbors), nearest_indices]
        print(f"vector_diffs shape: {vector_diffs.shape}")

        scalar_diffs = distances[batch_indices, torch.arange(N).view(1, -1, 1).expand(B, -1, num_neighbors), nearest_indices]
        print(f"scalar_diffs shape (before unsqueeze): {scalar_diffs.shape}")

        scalar_diffs = scalar_diffs.unsqueeze(-1)
        print(f"scalar_diffs shape (after unsqueeze): {scalar_diffs.shape}")

        # Concatenate vector and scalar differences to form geometric features.
        geom_features = torch.cat((vector_diffs, scalar_diffs), dim=-1)
        print(f"geom_features shape (after concat): {geom_features.shape}")
    ...
    # Concatenate all batches to form the final geometric features and nearest indices.
    geom_features = torch.cat(geom_features_list, dim=0)
    nearest_indices = torch.cat(nearest_indices_list, dim=0)

    print(f"Final geom_features shape: {geom_features.shape}")
    print(f"Final nearest_indices shape: {nearest_indices.shape}")
